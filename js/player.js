...
Audio.addEventListener("timeupdate", (event)=>{
	// console.log(event)
	const initialTime = event.target.currentTime; // Get current music time
	const finalTime = event.target.duration; // music duration
	let barWidth = (initialTime / finalTime) * 100;
	progressBar.style.width = `${barWidth}%`;

	progressArea.addEventListener("click", (event)=>{
		let progressValue = progressArea.clientWidth; //Get width of progress bar
		let clickedOffsetX = event.offsetX; //get offset x value
